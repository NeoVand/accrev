/**
 * Adapted from NeoVand/voicebook (MIT) and Supertone's browser reference
 * implementation (MIT). See THIRD_PARTY_NOTICES.md.
 */
...
import { MODEL_ASSET_CACHE } from './model-assets';
...
export class SupertonicAdapter {
	...
	async load(backend: Backend, progress: ProgressCallback): Promise<void> {
		...
		if (!this.policy.skipWarmup) {
			...
			try {
				...
				await this.infer('Hello, this is a quick warmup.', 'en', style, undefined, 1, 1.05);
			} catch (error) {
				console.warn(
	...
	private async infer(
		...
		try {
			...
			// Supertonic controls speaking pace by scaling the predicted duration
			// before latent generation. This preserves pitch and also reduces work
			// for faster voices, unlike AudioBufferSource playback-rate changes.
