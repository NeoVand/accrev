/**
 * Resumable model download cache adapted from NeoVand/voicebook (MIT).
 * See THIRD_PARTY_NOTICES.md.
 */
const DEFAULT_CHUNK_BYTES = 16 * 1024 * 1024;
const CHUNK_QUERY = 'accrev-model-chunk';
...
function cachedChunkIsValid(response: Response | undefined, expectedSize: number): boolean {
	...
	return (
		...
		response.headers.get('x-accrev-model-chunk') === 'v1'
	);
}
...
async function cacheChunk(
	...
	headers.set('x-accrev-model-chunk', 'v1');
